...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  //  钩子是图片上传成功之前触发，上传文件之前可以约束文件类型与大小
  // 要求：上传文件格式png | jpg |gif 小于4M
...
const validatorTmName = (rule: any, value: any, callback: any) => {
  ...
  if (value.trim().length >= 2) {
    ...
    callback(new Error('品牌名称字数至少两位'))
  }
}

// 品牌dialog对话框校验规则方法---品牌logo
// @ts-ignore
...
// 表单校验规则对象
const rules = {
  // required：表示必须校验，表单前出现五角星
  // trigger:触发校验的方式[blur,change]
